...
float pid_update_with_default_is_first(struct Pid *pid, float error, float dt) {
  pid->previous = error;
  pid->update = pid_update_with_default;
  return pid_update_with_default(pid, error, dt);
}
